...
def do_deploy(archive_path):
    ...
    file_result_1 = file_result[1].split(".")
    run('mkdir -p /data/web_static/releases/{}/'.format(file_result_1[0]))
    run('tar -xzf /tmp/{}.tgz -C /data/web_static/releases/{}/'
        .format(file_result_1[0], file_result_1[0]))
    run('rm /tmp/{}.tgz'.format(file_result_1[0]))
    run('mv /data/web_static/releases/{}/web_static/* '.format(file_result_1[0]) +
        '/data/web_static/releases/{}/'.format(file_result_1[0]))
    run('rm -rf /data/web_static/releases/{}/web_static'.format(file_result_1[0]))
    run('rm -rf /data/web_static/current')
    run('ln -s /data/web_static/releases/{}/ /data/web_static/current'
        .format(file_result_1[0]))
